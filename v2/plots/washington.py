...
from v2 import defaults, constants
from v2 import util
from v2.models import ExpectedFamilyContributionModel, PellGrantModel, InstitutionModel, StateAppropriationModel, \
    EnrollmentModel, PovertyGuidelineModel, GrantsScholarshipsModel, AffordableDebtModel
from v2.plots.base_plot import BasePlot
from v2.plots.common_utils import fetch_min_wage, compute_time_to_graduate
from v2.plots.compress import Smoothen
from v2.scenario import KEY_STATE_NEED_GRANT_MAX_AWARD, \
    KEY_STATE_NEED_GRANT_DISTRIBUTION_SCHEDULE
from v2.transform import import_poverty_guideline, import_state_need_grant_max_award, \
...
class StateNeedGrantPlot(BasePlot):
    ...
    def compute(self):
        self.set_up(self.scenario)
        plot_data = self.plot()
        smooth = constance.config.ENABLE_PLOT_SMOOTH_SWITCH and self.scenario.get(constants.PLOT_SMOOTHING)
...
class PellGrantPlot(BasePlot):
    ...
    def compute(self):
        self.set_up_signature(self.scenario)
...
class MinimumWagePlot(BasePlot):
    ...
    def compute(self):
        ...
        self.scenario[constants.ANNUAL_TAKE_HOME_PAY] = take_home_pay
...
class StateAppropriation(BasePlot):
    ...
    def compute(self):
        ...
        self.scenario[constants.TOTAL_APPROPRIATION] = total_appropriation
        self.scenario[constants.TOTAL_ENROLLMENT] = total_enrollment
        self.scenario[constants.STATE_APPROPRIATION] = appropriation_per_student
        return {
            constants.TYPE: constants.POINTS,
            constants.DATA: plot_data
        }
...
class InstitutionalAid(BasePlot):
    ...
    def compute(self):
        self.set_up(self.scenario)
        plot_data = []
        sng_shown = self.scenario.get(constants.SNG_SHOWN, True)
...
class AffordableDebtPlot(BasePlot):
    ...
    def compute(self):
        ...
        self.scenario[constants.EARNINGS_ANNUAL] = a1_dollar_value
